...
class WordExtractor:
    def __init__(
        ...
        self.ignore_blank_chars = ignore_blank_chars
    ...
    def iter_chars_to_words(
        ...
        # print('*'*20)
        # print("".join([t['text'] for t in ordered_chars]))
        for char in ordered_chars:
            text = char["text"]
            if self.ignore_blank_chars and text.isspace() and text.isspace():
                continue
    ...
    def iter_chars_to_lines(
        ...
        chars = list(chars)
        # print('*'*20)
        # print("".join([t['text'] for t in chars]))
        ...
        # Cluster by line
        # preserve_order: True保证语义的连贯性，如双栏pdf时，是按照半行半行阅读
    ...
    def iter_extract_tuples(
        ...
        for keyvals, char_group in grouped_chars:
            ...
            for line_chars, direction in line_groups:
                if idx < 100:
                    # print(idx,[(t['text'],t['top']) for t in line_chars])
                    print(idx,[(t['text']) for t in line_chars])
                idx += 1
                for word_chars in self.iter_chars_to_words(line_chars, direction):
                    # print([(t['text']) for t in word_chars])
